...
ClassDeclaration(path: Path): void {
  ...
  /**
   * 
   * added a classProperty traversal to check for static declarations, *these are another way of using Context API to pass context to *individual components, in this case :
   *class BookList extends Component {
   *static contextType = ThemeContext;
   *render()  { .... }   
   * we would change this to useContext...
   * 
   */
  ...
  path.traverse({
    ClassMethod(path: Path): void {
      ...
      // console.log(`cdm is: ${cdm}, cdu is ${cdu}, cwu is ${cwu}, render is ${render}, constructor is ${constructor}`)
      ...
      if(render) {
        // console.log('path.node.body.body[0] is: ', path.node.body.body[0])
